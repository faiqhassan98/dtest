...
export async function listTests(_req: Request, res: Response) {
  ...
  const mapped = tests.map((t) => ({
    ...t,
    totalQuestions: (t.passages || []).reduce(
      (s: number, p: any) => s + (p.questions?.length || 0),
      0
    ),
  }));
  res.json(mapped);
...
export async function updateTest(req: Request, res: Response) {
  const { id } = req.params;
  const { level, passages, isPublished } = req.body;
  const test = await ReadingTest.findByIdAndUpdate(
    id,
    { level, passages, isPublished },
    { new: true }
  );
  if (!test) return res.status(404).json({ message: "Not found" });
...
export async function nextTest(req: Request, res: Response) {
